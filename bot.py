    ...
    if result[0] == 0:
        if message.chat.id in admin:
            ...
            inline_buttons = [
                ...
                types.InlineKeyboardButton("Сайт", url="https://t.me/viezdizukkraine")
            ]
...
async def inline_button_handler(call: types.CallbackQuery):
    if call.data == 'rules':
        await call.message.edit_text("Без спама/флуда;") # Тут правила написать можно
    elif call.data == 'about':
        await call.message.edit_text("Мы энтузиасты, которые хотят помочь!") # Ну а тут просто кто вы и что вы
